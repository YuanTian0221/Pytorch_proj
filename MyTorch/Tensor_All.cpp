...
std::shared_ptr<Tensor<T>> Tensor<T>::operator-(const std::shared_ptr<Tensor<T>>& other) {
    ...
    out->_backward = [this, other, out]() {
        ...
        if (other->requires_grad) {
            for (size_t i = 0; i < other->data.size(); ++i) {
                other->grad[i] -= out->grad[i]; // 注意这里是减法的反向传播
                // 输出梯度计算的细节
                // std::cout << "this data: " << this->data[i] << ", other data: " << other->data[i] << ", output grad: " << out->grad[i] << std::endl;
                // std::cout << "Updating this grad: " << this->grad[i] << std::endl;
            }
...
std::shared_ptr<Tensor<T>> Tensor<T>::operator/(const std::shared_ptr<Tensor<T>>& other) {
    ...
    out->_backward = [this, other, out]() {
        if (this->requires_grad) {
            for (size_t i = 0; i < this->data.size(); ++i) {
                this->grad[i] += (1 / other->data[i]) * out->grad[i];  // 对于this的梯度，除法对应的导数为 1 / other
                //std::cout << "this data: " << this->data[i] << ", other data: " << other->data[i] << ", output grad: " << out->grad[i] << std::endl;
                //std::cout << "Updating this grad: " << this->grad[i] << std::endl;
            }
        }
        if (other->requires_grad) {
            for (size_t i = 0; i < other->data.size(); ++i) {
                other->grad[i] += (-this->data[i] / (other->data[i] * other->data[i])) * out->grad[i];  // 对于other的梯度，导数是 -this / other^2
                //std::cout << "this data: " << this->data[i] << ", other data: " << other->data[i] << ", output grad: " << out->grad[i] << std::endl;
                //std::cout << "Updating other grad: " << other->grad[i] << std::endl;
            }
        }
    };
